-- =============================================================================
-- ROLLBACK for: 20260726070000_fomo_company_age.sql
--   psql "$DATABASE_URL" -f supabase/rollback/20260726070000_fomo_company_age_down.sql
--
-- Drops the startup_id link and the pts_age term, restoring the view to its
-- 20260726060000 shape. The sourcing data itself is untouched; only the
-- founding-year enrichment and its scoring contribution go away.
-- =============================================================================

DROP VIEW IF EXISTS sourcing_fomo_scores;
DROP INDEX IF EXISTS idx_sourcing_companies_startup_id;
ALTER TABLE sourcing_companies DROP COLUMN IF EXISTS startup_id;

-- Recreate the pre-age view (see 20260726060000_fomo_gtm_signals.sql).
  ...
  SELECT p.company_id,
         ...
         count(DISTINCT s.signal_value) FILTER (WHERE s.signal_type='keyword'
           AND s.signal_value IN ('Sales Org','Customer Org','Finance Org','People Org','Compliance Org',
                                  'Enterprise GTM','Territory Org','Partnerships Org')) AS scaled_org_functions,
         count(DISTINCT s.signal_value) FILTER (WHERE s.signal_type='tech_stack') AS distinct_tech
  FROM job_signals s JOIN early_job_postings p ON p.id=s.job_id GROUP BY p.company_id
),
scored AS (
