-- Rollback for 20260827000000_update_alphamap_score_formula.sql
-- Restores calculate_alphamap_score() to the previous 3-pillar dual-track
-- definition from 20260719000300_dual_track_scoring.sql, verbatim.
...
BEGIN
  ...
  -- ══════════════════════════════════════════════════════════════════════════
  -- PILLAR A: Capital Efficiency (weight 45% venture / 20% mature)
  -- ══════════════════════════════════════════════════════════════════════════
  IF v_latest_valuation IS NOT NULL AND v_prev_valuation IS NOT NULL AND v_latest_amount IS NOT NULL THEN
  ...
  -- ══════════════════════════════════════════════════════════════════════════
  -- PILLAR B — venture_backed: Talent Velocity | mature_private: Scale & Longevity
  -- ══════════════════════════════════════════════════════════════════════════
  IF v_archetype = 'mature_private' THEN
    v_scale_score := CASE
  ...
  -- ══════════════════════════════════════════════════════════════════════════
  -- PILLAR C — Ecosystem Signal (investor quality); mature track blends in M&A
  -- ══════════════════════════════════════════════════════════════════════════
  ...
  ELSE
    ...
    ELSE
      v_pillar_c := (
          v_n_tier1 * 95.0 + v_n_tier2 * 75.0 + v_n_tier3 * 55.0
  ...
  -- ══════════════════════════════════════════════════════════════════════════
  -- Weight redistribution + macro adjustment
  -- ══════════════════════════════════════════════════════════════════════════
  ...
  v_wa := v_wa / v_total_w;  v_wb := v_wb / v_total_w;  v_wc := v_wc / v_total_w;
  ...
  -- sector_id keyword classifier (industry → sector_id) — unchanged from v1
  ...
  RETURN jsonb_build_object(
    ...
    'confidence',        CASE v_pillar_count WHEN 3 THEN 'high' WHEN 2 THEN 'medium' ELSE 'low' END,
    ...
    'pillars', jsonb_build_object(
      'capital_efficiency', jsonb_build_object(
        'label',  'Capital Efficiency', 'weight', ROUND((v_wa * 100)::numeric, 0),
        'score',  CASE WHEN v_a_valid THEN ROUND(v_pillar_a::numeric, 1) ELSE NULL END, 'valid', v_a_valid,
      ...
      'talent_velocity', jsonb_build_object(
        'label',  v_label_b, 'weight', ROUND((v_wb * 100)::numeric, 0),
        'score',  CASE WHEN v_b_valid THEN ROUND(v_pillar_b::numeric, 1) ELSE NULL END, 'valid', v_b_valid,
        'detail', CASE WHEN v_archetype = 'mature_private'
          THEN jsonb_build_object('headcount', v_employee_count, 'years_active', v_years_active,
                                   'stability', COALESCE(v_growth_trend, 'unknown'),
                                   'scale_score', ROUND(COALESCE(v_scale_score, 0)::numeric, 1))
          ELSE jsonb_build_object('hc_growth_pct', ROUND(COALESCE(v_hc_growth_pct, 0)::numeric * 100.0, 1),
                                   'serial_founder', v_is_serial_founder)
        END
      ),
      'ecosystem_signal', jsonb_build_object(
        'label',  v_label_c, 'weight', ROUND((v_wc * 100)::numeric, 0),
        'score',  CASE WHEN v_c_valid THEN ROUND(v_pillar_c::numeric, 1) ELSE NULL END, 'valid', v_c_valid,
        'detail', jsonb_build_object(
